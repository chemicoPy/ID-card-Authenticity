...
for i in range (0,image1.shape[0]):
    for j in range (0, image1.shape[1]):
        feature_matrix_image1[i][j] = ((int(image1[i,j,0]) + int(image1[i,j,1]) + int(image1[i,j,2]))/3)
...
#imshow(image1_test)
    ...
    for p in range (0,image1_test.shape[0]):
        for q in range (0, image1_test.shape[1]):
            feature_matrix_image1_test[p][q] = ((int(image1_test[p,q,0]) + int(image1_test[p,q,1])
            + int(image1_test[p,q,2]))/3)
...
#print(feature_matrix_image1_test)   # You can always uncomment this
    ...
    feature_sample_image_test1 = np.reshape(padded_array,(image1_shape[0] * image1_shape[1]))
...
#print(feature_sample_image_test1.shape)
    ...
    print('Cosine similarity:', cosine_similarity([feature_sample],[feature_sample_image_test1]))
    ...
    for similarity in cosine_similarity([feature_sample],[feature_sample_image_test1]):
        ...
        if image_test.endswith(".jpg"):
            ...
            print(str(image_test), "'s", "card genuinity percentage is:", (float(similarity)*100),"%")
